...
SRCS =	main.c sim-fast.c sim-safe.c sim-cache.c sim-profile.c sim-fastone.c\
	...
	eio.c stats.c endian.c misc.c ring_buffer.c capture_buf.c retire_trace.c\
	target-riscv64/riscv.c target-riscv64/loader.c target-riscv64/syscall.c \
	target-riscv64/symbol.c

HDRS =	syscall.h memory.h regs.h sim.h loader.h cache.h bpred.h ptrace.h retire_trace.h ring_buffer.h capture_buf.h\
...
sim-outorder$(EEXT):	sysprobe$(EEXT) sim-outorder.$(OEXT) cache.$(OEXT) bpred.$(OEXT) resource.$(OEXT) ptrace.$(OEXT) ring_buffer.$(OEXT) capture_buf.$(OEXT) retire_trace.$(OEXT)  $(OBJS) libexo/libexo.$(LEXT)
	$(CC) -o sim-outorder$(EEXT) $(CFLAGS) sim-outorder.$(OEXT) cache.$(OEXT) bpred.$(OEXT) resource.$(OEXT) ptrace.$(OEXT) ring_buffer.$(OEXT) capture_buf.$(OEXT) retire_trace.$(OEXT) $(OBJS) libexo/libexo.$(LEXT) $(MLIBS)
...
sim-outorder.$(OEXT): sim.h ring_buffer.h capture_buf.h retire_trace.h
