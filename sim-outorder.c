...
#define ISCOMPRESS(SZ)							\
  (compress_icache_addrs ? ((SZ) >> 1) : (SZ))
    ...
    #define ISCOMPRESS(I)        (INST_IsCompressed(I) ? sizeof(md_c_inst_t) : sizeof(md_inst_t)) 
#else /* !TARGET_PISA && !TARGET_RISCV */
    #define IACOMPRESS(A)        (A)
    #define ISCOMPRESS(SZ)        (SZ)
#endif /* TARGET_PISA */

// #define isAQ(INST) ((INST >> 26) & 0x1)
// #define isRL(INST) ((INST >> 25) & 0x1)

#define MACRO_FUSION_ORDER(MF_SET, ORDER, INST, OP, CPC, O1, O2, O3,   \
I1, I2, I3, I4, IMM, SRI, SM, SEQ, PTSEQ)  \
    (MF_SET[ORDER].IR = INST, MF_SET[ORDER].op = OP,    \
    MF_SET[ORDER].inst_PC = CPC,    \
    MF_SET[ORDER].out1 = O1, MF_SET[ORDER].out2 = O2, MF_SET[ORDER].out3 = O3,  \
    MF_SET[ORDER].in1 = I1, MF_SET[ORDER].in2 = I2, \
    MF_SET[ORDER].in3 = I3, MF_SET[ORDER].in4 = I4, \
    MF_SET[ORDER].imm = IMM, \
    MF_SET[ORDER].stack_recover_idx = SRI, MF_SET[ORDER].spec_mode = SM,    \
    MF_SET[ORDER].seq = SEQ, MF_SET[ORDER].ptrace_seq = PTSEQ)

#define INIT_MACRO_FUSION_ORDER(MF_SET) \
        if(MF_SET[SECOND].op != 0)  \
            memset(&MF_SET, 0, sizeof(MF_SET));  \

#define MACRO_FUSION_GENERAL_SET(MF, FIRST, LAST)   \
    (inst = MF[FIRST].IR,   \
    op = MF[LAST].op, \
    regs.regs_PC = MF[LAST].inst_PC,   \
    stack_recover_idx = MF[FIRST].stack_recover_idx,    \
    spec_mode = MF[FIRST].spec_mode,   \
    inst_seq = MF[FIRST].seq,   \
    ptrace_seq = MF[FIRST].ptrace_seq)
...
/* total number of memory page cross referneces */
...
sim_reg_options(struct opt_odb_t *odb) {
    ...
    opt_reg_string(odb, "-tlb:itlb",
                   "instruction TLB config, i.e., {<config>|none}",
                   &itlb_opt, "itlb:64:2048:4:l", /* print */TRUE, NULL);

    opt_reg_string(odb, "-tlb:dtlb",
                   "data TLB config, i.e., {<config>|none}",
                   &dtlb_opt, "dtlb:64:2048:4:l", /* print */TRUE, NULL);
    ...
    opt_reg_int(odb, "-res:fpmult_acc",
                "total number of floating point multiply accumulate available",
                &res_fpmult, /* default */fu_config[FU_FPMULT_ACC_INDEX].quantity,
            /* print */TRUE, /* format */NULL);
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
    ...
    stat_reg_counter(sdb, "sim_num_comp_insn",
                     "total number of compressed instructions committed",
                     &sim_num_comp_insn, sim_num_comp_insn, NULL);
    stat_reg_formula(sdb, "sim_insn_comp_rate",
                     "compressed rate (i.e., comp insn/total insn)",
                     "sim_num_comp_insn / sim_num_insn", NULL);
    stat_reg_formula(sdb, "sim_code_size_reduction",
                     "code size reduction via enable extension C",
                     "(sim_num_comp_insn / sim_num_insn) / 2", NULL);
    stat_reg_counter(sdb, "sim_num_macro_fusion_set",
                     "total number of macro fusion set committed",
                     &sim_num_macro_fusion_set, sim_num_macro_fusion_set, NULL);
    stat_reg_counter(sdb, "sim_num_fused_inst",
                     "total number of fused instruction",
                     &sim_num_fused_inst, sim_num_fused_inst, NULL);
    stat_reg_formula(sdb, "sim_average_macro_fusion",
                     "average of instruction fused to a macro",
                      "(sim_num_fused_inst / sim_num_macro_fusion_set)", NULL);
    stat_reg_formula(sdb, "sim_macro_fusion_precentage",
                     "precentage of macro fusion against total instrcution count",
                      "(sim_num_fused_inst / sim_num_insn)", NULL);
    ...
    stat_reg_counter(sdb, "mem_cross_page_refs",
                     "total number of page crossing references",
                     &mem_cross_page_refs, mem_cross_page_refs, NULL);
}
...
struct RUU_station {
    ...
    md_addr_t PC, next_PC, pred_PC, mf_firstPC;    /* inst PC, next PC, predicted PC, first PC of macro */
...
struct RUU_station_buffer {
    md_inst_t IR;            /* instruction bits, depends on first inst*/
    enum md_opcode op;            /* decoded instruction opcode, depends on macro */
    md_addr_t inst_PC;    /* inst PC , depends on first inst*/
    int out1, out2, out3;           /* output register names for macro fusion */
    int in1, in2, in3, in4;         /* input register names for macro fusion */
    ext_unsigned_t imm;           /* temp immdiate for macro fusion */
    int stack_recover_idx;        /* non-speculative TOS for RSB pred, depends on first inst */
    int spec_mode;            /* non-zero if issued in spec_mode, depends on macro */
    INST_SEQ_TYPE seq;            /* instruction sequence, used to
					                    sort the ready list and tag inst, depends on first inst*/
    unsigned int ptrace_seq;        /* pipetrace sequence number, depends on first inst*/
};

/* non-zero if all register operands are ready, update with MAX_IDEPS */
#define OPERANDS_READY(RS)                                              \
  ((RS)->idep_ready[0] && (RS)->idep_ready[1] && (RS)->idep_ready[2] && (RS)->idep_ready[3])
...
if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|++++Enter ruu_release_fu　　      |\n");
                fprintf(stderr, "===================================\n");
            }
    ...
    if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|           exit ruu_release_fu----|\n");
                fprintf(stderr, "===================================\n");
            }
}
...
readyq_enqueue(struct RUU_station *rs)        /* RS to enqueue */
{
    ...
    static int _i = 0;
    /* node is now queued */
    // debug("readyq_enqueue: %d", _i);
    // debug("instuction name: %s ; %x", MD_OP_NAME(rs->op), rs->PC);
    _i++;
    if (rs->queued)
    ...
    /* locate insertion point */
    if (!(MD_OP_FLAGS(rs->op) & F_ATOMIC) || rs->in_LSQ || MD_OP_FLAGS(rs->op) & (F_LONGLAT | F_CTRL)) {
...
cv_dump(FILE *stream)                /* output stream */
{
    ...
    for (i = 0; i < MD_TOTAL_REGS; i++) {
        ...
        fprintf(stream, "[DEBUG] cv%02d: rs=%p, in_LSQ=%d, idx=%d\n",
                i, ent.rs, ent.rs ? ent.rs->in_LSQ : -1, 
                ent.rs ? (int) (ent.rs - (ent.rs->in_LSQ ? LSQ : RUU)) : -1);
...
ruu_commit(void) {
    ...
    if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|++++Enter ruu_commit　　　　      |\n");
                fprintf(stderr, "===================================\n");
            }
    ...
    while (RUU_num > 0 && committed < ruu_commit_width) {
        ...
        if (pred
            ...
            bpred_update(pred,
                    /* branch address */(((rs->mf_firstPC) != 0) ? rs->mf_firstPC : regs.regs_PC),
...
if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|++++Enter ruu_writeback 　　      |\n");
                fprintf(stderr, "===================================\n");
            }
    ...
    while ((rs = eventq_next_event())) {
        ...
        if (rs->recover_inst) {
            ...
            ruu_recover(rs - RUU);
            tracer_recover();
            bpred_recover(pred, ((rs->mf_firstPC != 0 )? rs->mf_firstPC : rs->PC), rs->stack_recover_idx);
...
if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|            exit ruu_writeback----|\n");
                fprintf(stderr, "===================================\n");
            }
...
lsq_refresh(void) {
    ...
    if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|++++Enter lsq_refresh   　　      |\n");
                fprintf(stderr, "===================================\n");
            }
    ...
    for (i = 0, index = LSQ_head, n_std_unknowns = 0;
	 ...
	 as no later load could be resolved in its presence */
        if (/* store? */
                (MD_OP_FLAGS(LSQ[index].op) & (F_MEM | F_STORE)) == (F_MEM | F_STORE)  /*& (!(MD_OP_FLAGS(LSQ[index].op) & F_ATOMIC))*/ ) {
...
if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|              exit lsq_refresh----|\n");
                fprintf(stderr, "===================================\n");
            }
    ...
    for (n_issued = 0;
        ...
        if (RSLINK_VALID(node)) {
            ...
            if (rs->in_LSQ
                       ...
                       && !((MD_OP_FLAGS(rs->op) & (F_ATOMIC)) == (F_ATOMIC))) {
		 ...
		 (see ruu_commit()) */
                ...
                if ((rs->onames[0] || rs->onames[1] || rs->onames[2]) /*&& !(MD_OP_FLAGS(rs->op) & F_ATOMIC)*/) {           
                	if (rs->op != MD_NOP_OP) {
                        switch (rs->op) {                
...
#define DTMP            (1+1+32+32)
//#define DFCSR			(1 + 32 + 32)
#define DCSR(N)            (1 + 32 + 32)
// #define DCSR(N)			((N) + 1 + 32 + 32)
...
ruu_dispatch(void) {
    ...
    md_inst_t inst2, inst3;     /* actual instruction bits for macro fusion */
    enum md_opcode op;            /* decoded opcode enum */
    int out1, out2, out3, in1, in2, in3, in4;    /* output/input register names */
    int out1a, out2a, out3a, in1a, in2a, in3a, in4a;    /* input/output register names for macro fusion */
    int out1b, out2b, out3b, in1b, in2b, in3b, in4b;    /* input/output register names for macro fusion */
    ext_unsigned_t imm, imm2, imm3;     /* temp immdiate for macro fusion match */
    md_addr_t target_PC;            /* actual next/target PC address */
    md_addr_t addr;            /* effective address, if load/store */
    md_addr_t mf_firstPC;
    ...
    static int remaining_macro_inst = 0;
    static int macro_total_count = 0;
    static struct RUU_station_buffer macro_fusion_order[3] = {0};
...
if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|++++Enter ruu_dispatch  　　      |\n");
                fprintf(stderr, "===================================\n");
            }
    ...
    while (/* instruction decode B/W left? */
        ...
        regs.regs_NPC = regs.regs_PC + ISCOMPRESS(inst);
        ...
        if((MD_OP_FLAGS(op) & F_SYNC))
            if(RUU_num != 0)
                break;

        /* Fixme */
        // if(MD_OP_FLAGS(op) & F_AMO) {
        //     if(isAQ(inst) || isRL(inst))
        //         if(RUU_num != 0)
        //             break;
        // }
        ...
        if(verbose) fprintf(stderr, "Decoding & Executing inst\n");

        debug("%d\t, (%d) inst[%x], addr[%x], op[%d]", ++i_count, spec_mode, inst, regs.regs_PC, op);
...
#include "machine.def"
        ...
        /* MD_HINT_INST(op, out1, out2, out3); */
        ...
        br_taken = (regs.regs_NPC != (regs.regs_PC + ISCOMPRESS(inst)));
        br_pred_taken = (pred_PC != (regs.regs_PC + ISCOMPRESS(inst)));
        ...
        /* macro fusing order detector */
        if(macro_fusion_active) {

        if(op >= C_LUI_START && op <= C_LUI_END) op = C_LUI_OP; /* initilize to first c.lui opcode*/
        if(op >= C_ADD_START && op <= C_ADD_END) op = C_ADD_OP; /* c.add ditto*/
        if(op >= C_JALR_START && op <= C_JALR_END) op = C_JALR_OP; /* c.jalr ditto */

        /* record first inst of macro fusion to normally execute if no macro fusion match */
        MACRO_FUSION_ORDER(macro_fusion_order, FIRST, inst, op, 
        regs.regs_PC, out1, out2, out3, in1, in2, in3, in4, imm/*need?*/, 
        stack_recover_idx, spec_mode/*need?*/, inst_seq, pseq);

        if(/*!spec_mode &&*/ !remaining_macro_inst &&
        (((fetch_head + 1) & (ruu_ifq_size - 1)) != fetch_tail)){
            switch(op){
                case C_LUI_OP:
                case LUI_OP:
                    inst = fetch_data[(fetch_head + 1) & (ruu_ifq_size - 1)].IR;
                    MD_SET_OPCODE(op, inst);
                    switch(op){
                        case C_LD_OP:
                        case LD_OP:
                            /* // rd = *(imm[31:0])
                                lui rd, imm[31:12]
                                ld rd, imm[11:0](rd) */
                            #include"decode_inst2.def"
                            if((out1 != out1a) || (in2a != out1a))
                                break;                           
                            /* Current instruction that has just been decoded but not yet processed */
                            remaining_macro_inst++; 
                            macro_total_count++;
                            break;
                        case C_ADDI_OP:
                        case ADDI_OP:
                ...
                case C_LD_OP:
                case LD_OP:
                    if(op==C_LD_OP) imm = EXT(DIMM_C_BRLS);
                    if(op==LD_OP) imm = SEXT(IMM);
                    switch(op){                        
                        inst = fetch_data[(fetch_head + 1) & (ruu_ifq_size - 1)].IR;
                        MD_SET_OPCODE(op, inst);
                        case C_LD_OP:
                        ...
                        /* // ldpair rd1,rd2, [imm(rs1)]
                            ld rd1, imm(rs1)
                            ld rd2, imm+8(rs1) */  
                            if(op==C_LD_OP) imm2 = EXT(DIMM_C_BRLS);                 
                            if(op==LD_OP) imm2 = SEXT(IMM);
                            #include"decode_inst2.def"
                            if((out1 == out1a) || (in2 != in2a) || (imm2 != (imm + 8)))
                                break;        
                            macro_fusion_order[FIRST].imm = imm;
                            macro_fusion_order[FIRST].imm = imm2;    
                            remaining_macro_inst++; 
                            macro_total_count++;
                            break;
                        case C_ADDI_OP:
                        ...
                        /* // ldia rd, imm(rs1)
                            ld rd, imm(rs1)
                            addi rs1, rs1, 8 */
                            if(op==C_ADDI_OP) imm2 = SEXT_TO(IMM_C_5_4_0, 6, 64);                 
                            if(op==ADDI_OP) imm2 = SEXT(IMM);
                            #include"decode_inst2.def"
                            if((in2 != out1a) || (out1a != in2a) || (imm2 != sizeof(md_addr_t)))
                                break;
                            remaining_macro_inst++;
                            macro_total_count++;
                            break;
                ...
                case ADDIW_OP:
                    ...
                    switch(op){
                        case C_SLLI64_OP:
                        case SLLI_OP:
                            if((fetch_head + 2) & (ruu_ifq_size - 1) == fetch_tail)
                                break;
                            #include"decode_inst2.def"
                            if(op==C_SLLI64_OP) imm2 = (SHAMT_C);
                            if(op==SLLI_OP) imm2 = (SHAMT_I);
                            inst = fetch_data[(fetch_head + 2) & (ruu_ifq_size - 1)].IR;
                            MD_SET_OPCODE(op, inst);
                            if(op==C_SRLI64_OP) imm2 = (SHAMT_C);
                            if(op==SRLI_OP) imm2 = (SHAMT_I);
                            switch(op) {
                                case C_SRLI64_OP:
                                case SRLI_OP:
                                /*  addiw rd, rs1, imm12
                                    slli rd, rs1, 32
                                    SRLI rd, rs1, 32 */
                                    #include"decode_inst3.def"
                                    if((out1 != out1a) || (out1a != out1b) ||
                                        (in2 != in2a) || (in2a != in2b) ||
                                        (imm2 != 32) || (imm3 != 32))
                                        break;                           
                                    remaining_macro_inst += 2; 
                                    macro_total_count += 2;
                                    break;//end of third, srli
                                default:
                                    break;
                            }
                            break;// end of second, slli
                        default:
                            break;
                    }
                    break;// end addiw
                        ...
                        case ADD_OP:
                            ...
                            switch(op) {
                                ...
                                /* // rd = array[offset]
                                    ...
                                    ld rd, 0(rd) */
                                    #include"decode_inst3.def"
                                    if((out1a != out1b) || (out1a != in2a) || (out1b != in2b))
                                        break;
                                    remaining_macro_inst++;
                                    macro_total_count++;
                                    break;
        ...
        /* if had macro inst, continue to next inst until do last inst of macro fusion set,
            else  means macro fusion not work, recover it and normally dispatching */
        if(remaining_macro_inst > 0) {
            /* if the instruction fetch queue still has entries to pop, 
                it indicates that the current instruction belongs to a macro fusion order. Record it. */
            int cur_macro_idx = macro_total_count - remaining_macro_inst;
            MACRO_FUSION_ORDER(macro_fusion_order, cur_macro_idx, inst, op, 
                regs.regs_PC, out1, out2, out3, in1, in2, in3, in4, imm/*need?*/, 
                stack_recover_idx, spec_mode/*need?*/, inst_seq, pseq);
            remaining_macro_inst--;
            /* dispatch stage continue to execute next inst of one macro fusion set */
            fetch_head = (fetch_head + 1) & (ruu_ifq_size - 1);
            continue;
        }
        else { 
            inst = macro_fusion_order[FIRST].IR;
            op = macro_fusion_order[FIRST].op;
        }
        
        /* do macro fusing for renaming register if macro set had record macro inst, */
        if(macro_total_count > 1) {   
            mf_firstPC = macro_fusion_order[FIRST].inst_PC;     
            switch(macro_fusion_order[FIRST].op) {
                case C_LUI_OP:
                case LUI_OP:
                    switch(macro_fusion_order[SECOND].op) {
                        case C_LD_OP:
                        case LD_OP:
                            /* // rd = *(imm[31:0]) <=> ld rd, imm[31:0]
                                lui rd, imm[31:12]
                                ld rd, imm[11:0](rd) */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            /* input/output not change */
                            break;                   
                        case C_ADDI_OP:         
                        case ADDI_OP:
                            /* // rd = imm[31:0] <=> addi rd, rd, imm[31:0]
                                lui rd, imm[31:12]
                                addi rd, rd, imm[11:0] */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            /* input/output not change */
                ...
                case AUIPC_OP:
                     switch(macro_fusion_order[SECOND].op) {
                        ...
                        case C_JALR_OP:
                        case JALR_OP:
                        /* // far jump (1 MB) (AUIPC+JALR) <=> jalr ra, imm32
                            auipc t, imm20
                            jalr ra, imm12(t) */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            in2 = NA;
                ...
                case LD_OP:
                    switch(macro_fusion_order[SECOND].op) {
                        ...
                        /* // ldpair rd1,rd2, [imm(rs1)]
                            ...
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            /* input/output not change */
                            out1 = macro_fusion_order[FIRST].out1;
                            out2 = macro_fusion_order[SECOND].out1;
                            break;
                        case C_ADDI_OP:
                        case ADDI_OP:
                        /* // ldia rd, imm(rs1)
                ...
                case ADDIW_OP:
                    switch(macro_fusion_order[SECOND].op) {
                        case C_SLLI64_OP:
                        case SLLI_OP:
                            switch(macro_fusion_order[THIRD].op) {
                                case C_SRLI64_OP:
                                case SRLI_OP:
                                /*  addiw rd, rs1, imm12
                                    slli rd, rs1, 32
                                    SRLI rd, rs1, 32 */
                                    MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, THIRD);
                                    /* input/output not change */
                                    break;
                                default:
                                    break;
                ...
                case SLLI_OP:
                    switch(macro_fusion_order[SECOND].op) {
                        case C_SRLI64_OP:
                        case SRLI_OP:
                            /* // rd = rs1 & 0xffffffff <=> and rd, rs1, 0xffffffff
                                slli rd, rs1, 32
                                srli rd, rd, 32 */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            op = AND; /* and */
                            in2 = macro_fusion_order[FIRST].in2;
                            break;
                        case C_ADD_OP:
                        case ADD_OP:
                            /* // &(array[offset]) <=> add, rd, shift(rd), rs2
                            slli rd, rs1, {1,2,3}
                            add rd, rd, rs2 */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            switch(macro_fusion_order[THIRD].op) {
                                case C_LD_OP:
                                case LD_OP:
                                /* // rd = array[offset] <=> ld rd, (shift(rd)+rs2)
                                    slli rd, rs1, {1,2,3}
                                    add rd, rd, rs2
                                    ld rd, 0(rd) */
                                    MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, THIRD);
                                    in1 = macro_fusion_order[FIRST].in2;
                                    in3 = macro_fusion_order[SECOND].in3;
                                    break;
                                default:
                                    break;
                            }
                ...
                case ADD_OP:
                    switch(macro_fusion_order[SECOND].op) {
                        case C_LD_OP:
                        case LD_OP:
                        /* // rd = array[offset] <=> ld rd, (rs1+rs2)
                            add rd, rs1, rs2
                            ld rd, 0(rd) */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            in1 = macro_fusion_order[FIRST].in1;
                            in2 = macro_fusion_order[FIRST].in2;
                            break;
                        default:
                            break;
                    ...
                    switch(macro_fusion_order[SECOND].op) {
                        ...
                        /* div[U] rdq, rs1, rs2
                           rem[U] rdr, rs1, rs2 */
                            MACRO_FUSION_GENERAL_SET(macro_fusion_order, FIRST, SECOND);
                            out1 = macro_fusion_order[FIRST].out1;
                            out2 = macro_fusion_order[SECOND].in1;
                            break;
                        default:
                            break;
                    }
        ...
        /* is this a NOP */
        if ((op != MD_NOP_OP || op != MD_C_NOP_OP) && (!remaining_macro_inst)) {
            /* for load/stores:
	   ...
	   */
            ...
            if (MD_OP_FLAGS(op) & F_MEM) {
                ...
                ruu_link_idep(rs, /* idep_ready[] index */3, in4); // FIXME check here
                ...
                if (((MD_OP_FLAGS(op) & (F_MEM | F_STORE)) == (F_MEM | F_STORE))
                    && OPERANDS_READY(lsq) && (((MD_OP_FLAGS(op) & F_LOAD) != F_LOAD))) {
	  ...
	  sim_num_insn++;
        ...
        if (op == MD_NOP_OP || op ==MD_C_NOP_OP) {
            /* end of the line */
            ptrace_endinst(pseq);
        }
        ...
        /* consume instruction from IFETCH -> DISPATCH queue */
        fetch_head = (fetch_head + 1) & (ruu_ifq_size - 1);
        for(int ifq_pop_cnt = 0; ifq_pop_cnt < macro_total_count; ifq_pop_cnt++)
            fetch_num--;
        if(macro_total_count > 1) {
            sim_num_macro_fusion_set++;
            sim_num_fused_inst += macro_total_count;
        }
        /* initilize to 1 */
        macro_total_count = 1;
        
        remaining_macro_inst = 0;
        INIT_MACRO_FUSION_ORDER(macro_fusion_order);
 ...
 if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|            exit ruu_dispatch----|\n");
                fprintf(stderr, "===================================\n");
            }
...
#define MATCH_MAX_IFQ_SIZE ((ruu_ifq_size / (ruu_decode_width * fetch_speed)))
...
if(verbose) {
    ...
    for (i = 0, branch_cnt = 0;
        ...
        if (ld_text_base <= fetch_regs_PC
            ...
            && !(fetch_regs_PC & (sizeof(md_c_inst_t) - 1))) {
		 ...
		 speculative TLB misses */
            ...
            inst = MD_NOP_INST;
            // fprintf(stderr, "invaild fetch: %x\n", inst);
        }
        ...
        if (pred) {
	     ...
	     value may be 1 if bpred can only predict a direction */
            ...
            if (!fetch_pred_PC) {
                ...
                fetch_pred_PC = fetch_regs_PC + ISCOMPRESS(inst);
            } else {
    ...
    // fprintf(stderr, "i=%d [head:%d ; tail:%d ; fetch_num:%d]\n", i,  fetch_head, fetch_tail, fetch_num);
    // int idx = fetch_head;
    // for(; idx != ((fetch_tail - 1) & (ruu_ifq_size - 1)); idx = ((idx + 1) & (ruu_ifq_size - 1)))                  
    //     fprintf(stderr, "<%d> %x ; %x\n", idx, fetch_data[idx].regs_PC, fetch_data[idx].IR);
    // fprintf(stderr, "<%d> %x ; %x\n", idx, fetch_data[idx].regs_PC, fetch_data[idx].IR);
    if(verbose) {
                fprintf(stderr, "===================================\n");
                fprintf(stderr, "|               exit ruu_fetch----|\n");
                fprintf(stderr, "===================================\n");
            }
}
