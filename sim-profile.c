...
#include "regs.def"
/*
#define CSRS(N)                 (regs.regs_C.csr[N])
#define SET_CSRS(N,EXPR)        (regs.regs_C.csr[N] = (EXPR))

#define FPR(N)                  (regs.regs_F.f[(N)*2])
#define SET_FPR(N,EXPR)         (regs.regs_F.f[(N)*2] = (EXPR))
...
#include "machine.def"
	default:
	  panic("attempted to execute a bogus opcode %d", op);
      }
